...
// ─── ADDED: Browser Speech Recognition detection ───
...
// ADDED: helper for speech timestamps
...
export default function App() {
  ...
  // CHANGED: Recording timer — only timer, no fake levels when mic is active
  useEffect(() => {
  ...
  // CHANGED: Simulated transcription — only runs when Speech API is NOT available
  useEffect(() => {
    if (!recording || paused || recogRef.current) return;
    // No Speech API → fall back to simulation + fake levels
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      const poll = () => {
        ...
        setLiveLevel(buf.reduce((a, b) => a + b, 0) / buf.length / 255);
    ...
    // Start Web Speech API for real transcription
    if (SpeechRec) {
      const r = new SpeechRec();
      r.continuous = true;
      r.interimResults = false;
      r.lang = settings.language === "auto" ? "en-US" : (settings.language || "en");
      r._t0 = Date.now();
      ...
      r.onresult = (ev) => {
        for (let i = ev.resultIndex; i < ev.results.length; i++) {
          if (ev.results[i].isFinal) {
            ...
            if (txt.length > 1) {
              const sec = Math.floor((Date.now() - r._t0) / 1000);
      ...
      try { r.start(); recogRef.current = r; } catch (e) { console.warn("Speech start failed:", e); }
    }
  };

  // CHANGED: stopRecording — cleanup mic + speech recognition
  const stopRecording = () => {
    ...
    const m = {
      id: `m${Date.now()}`,
      title: `Meeting ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
      date: new Date().toISOString(),
      duration: elapsed,
      status: "completed",
      source: "recording",
      transcript: tr.length > 0 ? tr : [{ time: "00:00", speaker: "System", text: "No speech detected during recording." }],
      summary: null
    };
          ...
          <div className="border-b border-gray-800/80 px-5 py-3 flex items-center gap-3 shrink-0">
            {recording ? (
              <div className="flex items-center gap-3 flex-1">
                {/* CHANGED: shows (Mic) or (Simulated) */}
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500 animate-pulse" /><span className="text-sm font-semibold text-red-400">Recording {recogRef.current ? "(Mic)" : "(Simulated)"}</span><span className="text-sm text-gray-400 font-mono">{fmt(elapsed)}</span></div><div className="flex-1" />
            ...
            {!recording && !activeMeeting ? (
              ...
              <div className="p-5 space-y-1">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    {/* CHANGED: shows Live Mic or Simulated */}
                ...
                {/* CHANGED: better empty state for mic */}
                {!curT.length ? (<div className="flex flex-col items-center justify-center py-20 text-gray-600"><Mic size={28} className="mb-3 opacity-50" /><p className="text-sm">{recording ? (recogRef.current ? "Listening via microphone — speak now..." : "Simulated transcription...") : "No transcript"}</p></div>)
                  : (<div className="space-y-1">{curT.map((t, i) => (<div key={i} className="flex gap-3 py-2 px-3 rounded-lg hover:bg-gray-800/40">
                    ...
                    {/* CHANGED: better listening indicator */}
                    {recording && <div className="flex items-center gap-3 py-3 px-3 text-gray-600"><Loader2 size={14} className="animate-spin" /><span className="text-xs">{recogRef.current ? "Listening via microphone..." : "Simulated transcription..."}</span></div>}</div>)}
              </div>
            ) : (
              <div className="p-5">
                {generating ? (
